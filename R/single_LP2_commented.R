...
#' @param shortage_factor the value by which we multiply shortage (vs. waste)
...
single_lpSolve <- function(d, l1_bounds, lag_bounds, num_vars,
  ...
  ## w >=0, s >= 0, r1 >= 0, r2 >= 0 (This is important to prevent unboundedness)
  for (i in 1:N){
    ...
    ## Setting each coeff of s to be non-negative:
    constraint_coefficients <- rep(0, N_var)
    constraint_coefficients[s_idx0+i] <- 1
    lpSolveAPI::add.constraint(my.lp, constraint_coefficients, type = ">=", 0)

    ## Setting each coeff of r1 to be non-negative:
    ## r1_{i} >= 0
    constraint_coefficients <- rep(0,N_var)
  ...
  for (i in (start+1):N){
    ...
    constraint_coefficients <- rep(0,N_var)
    constraint_coefficients[s_idx0+i] <- 1
    constraint_coefficients[r1_idx0+i-1] <- 1
    constraint_coefficients[r2_idx0+i-1] <- 1
    constraint_coefficients[x_idx0+i] <- 1
    constraint_coefficients[w_idx0+i] <- -1
    lpSolveAPI::add.constraint(my.lp, constraint_coefficients, ">=", y[i])
